...
const CartItem = ({ item: { id, name, type, price, image, color, cartQuantity} }) => {
  const dispatch = useDispatch();

  const onRemoveItem = () => {
    dispatch(setRemoveItemFromCart({  id, name, type, price, image, color, cartQuantity }))
  }

  const onIncreaseItemQTY = () => {
    dispatch(setIncreaseItemQTY({ id, name, type, price, image, color, cartQuantity }))
  }
  const onDecreaseItemQTY = () => {
    dispatch(setDecreaseItemQTY({  id, name, type, price, image, color, cartQuantity }))
  }
    ...
    <>
      <div className="flex items-center justify-between w-full px-5">
        <div className="flex items-center gap-5">
          <div className={`bg-gradient-to-b ${color} ${name} relative rounded p-3 hover:scale-105 transition-all duration-75 ease-in-out grid items-center`}>
